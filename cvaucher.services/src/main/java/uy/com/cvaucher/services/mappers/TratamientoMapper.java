...
{
	@Select("SELECT * FROM tratamiento t, tipo_tratamiento tt "
	...
	@Select("SELECT * FROM tratamiento WHERE trat_id=#{tratId}")
	@ResultMap("uy.com.cvaucher.services.mappers.TratamientoMapper.TratResult")
	Tratamiento findTratamientoById(Integer tratId);
	
	@Select("SELECT * FROM tratamiento WHERE trat_cedula=#{cedula}")
	@ResultMap("uy.com.cvaucher.services.mappers.TratamientoMapper.TratResult")
